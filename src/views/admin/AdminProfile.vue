  ...
  <div class="max-w-2xl">
    ...
    <div class="card card-body">
      ...
      <form @submit.prevent="saveProfile" class="space-y-5">
        <div class="form-group">
          ...
          <input v-model="form.full_name" type="text" class="form-input" placeholder="Your name" />
        </div>
        <div class="form-group">
          <label class="form-label">Email Address</label>
          <input :value="authStore.profile?.email" type="email" class="form-input bg-gray-50 cursor-not-allowed" disabled />
          <p class="form-error text-gray-400">Email cannot be changed</p>
        </div>
        <div class="pt-2">
    ...
    <div class="card card-body mt-4">
      ...
      <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
        ...
        <button @click="showPasswordModal = true" class="btn-secondary btn-sm">Change Password</button>
      </div>
    </div>
  </div>
  ...
  <AppModal :show="showPasswordModal" title="Change Password" @close="showPasswordModal = false">
    <form @submit.prevent="changePassword" class="space-y-4">
      <div class="form-group">
        ...
        <input v-model="passwordForm.password" type="password" class="form-input" minlength="8" required placeholder="Min. 8 characters" />
      </div>
      <div class="form-group">
        <label class="form-label">Confirm Password</label>
        <input v-model="passwordForm.confirm" type="password" class="form-input" required placeholder="Repeat password" />
